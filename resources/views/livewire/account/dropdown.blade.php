<x-shopfolio::dropdown align="left" width="56" customAlignmentClasses="right-1" containerClasses="relative" contentClasses="bg-white dark:bg-secondary-800 text-left">
    <x-slot name="trigger">
    ...
    <x-slot name="content">
        <div class="px-4 py-3">
            <p class="text-sm leading-5 txt-secondary-900 dark:text-white">
                {{ __('shopfolio::layout.account_dropdown.sign_in') }}
            </p>
        ...
        @can('add_products')
            ...
            <div class="py-1">
                <x-shopfolio::dropdown-link :href="route('shopfolio.products.create')">
                ...
                </x-shopfolio::dropdown-link>
            </div>
        @endcan
        <div class="border-t border-secondary-100 dark:border-secondary-700"></div>
        <div class="py-1">
            <x-shopfolio::dropdown-link :href="route('shopfolio.profile')">
                <svg class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500 group-focus:text-secondary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                {{ __('shopfolio::layout.account_dropdown.personal_account') }}
            </x-shopfolio::dropdown-link>
            @can('view_users')
                <x-shopfolio::dropdown-link :href="route('shopfolio.settings.users')">
                    <svg class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500 group-focus:text-secondary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                    </svg>
                    {{ __('shopfolio::layout.account_dropdown.manage_users') }}
                </x-shopfolio::dropdown-link>
            @endcan
        </div>
        <div class="border-t border-secondary-100 dark:border-secondary-700"></div>
        <div class="py-1">
            @can('access_setting')
                <x-shopfolio::dropdown-link :href="route('shopfolio.settings.index')">
                    <svg class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500 group-focus:text-secondary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    {{ __('shopfolio::layout.account_dropdown.settings') }}
                </x-shopfolio::dropdown-link>
            @endcan
        </div>
        <div class="border-t border-secondary-100 dark:border-secondary-700"></div>
        <div class="py-1">
            <x-shopfolio::dropdown-link
                :href="route('shopfolio.logout')"
                onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
            >
                <svg class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500 group-focus:text-secondary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                </svg>
                {{ __('shopfolio::layout.account_dropdown.sign_out') }}
            </x-shopfolio::dropdown-link>
            <form id="logout-form" action="{{ route('shopfolio.logout') }}" method="POST" style="display: none;">
                @csrf
            </form>
        </div>
    </x-slot>
