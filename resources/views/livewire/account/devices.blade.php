    ...
    <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
            <div class="px-4 sm:px-0">
                <h3 class="text-lg font-bold leading-6 text-secondary-900 dark:text-white">{{ __('shopfolio::pages/auth.account.device_title') }}</h3>
                <p class="mt-4 text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                    {{ __('shopfolio::pages/auth.account.device_description') }}
                </p>
            </div>
        </div>
                ...
                <div class="px-4 py-5 sm:p-6">
                    @if (count($this->sessions) > 0)
                        <p class="text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                            {{ __('shopfolio::pages/auth.account.empty_device') }}
                        </p>
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <div class="flex items-center">
                                                ...
                                                @if ($session->is_current_device)
                                                    <span class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-green-100 text-green-800">
                                                        {{ __('shopfolio::pages/auth.account.current_device') }}
                                                    </span>
                                                @else
                                                    <span class="ml-2 text-xs text-secondary-400 dark:text-secondary-500">{{ __('shopfolio::pages/auth.account.device_last_activity') }} {{ $session->last_active }}</span>
                                                @endif
                                            </div>
                                            <p class="mt-0.5 text-sm leading-4 text-secondary-500 dark:text-secondary-400">
                                                ...
                                                @else
                                                    {{ __('shopfolio::pages/auth.account.device_location') }}
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                    @if (!$session->is_current_device)
                                        <div class="ml-4">
                                            <span class="inline-flex rounded-md shadow-sm">
                                                <x-shopfolio::buttons.primary wire:click="$emit('openModal', 'shopfolio-modals.logout-others-browser')" wire:loading.attr="disabled" type="button">
                                                    {{ __('shopfolio::messages.log_out') }}
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm leading-5 text-yellow-700">
                                        {{ __('shopfolio::pages/auth.account.device_enabled_feature') }}
