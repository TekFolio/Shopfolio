<div>
    <div>
        <div>
            <h3 class="text-lg leading-6 font-medium text-secondary-900 dark:text-white">
                {{ __('shopfolio::pages/products.inventory.title') }}
            </h3>
            <p class="mt-1 max-w-2xl text-sm text-secondary-500 dark:text-secondary-400">
                {{ __('shopfolio::pages/products.inventory.description') }}
            </p>
        </div>
        <div class="mt-5 bg-white dark:bg-secondary-800 rounded-lg overflow-hidden shadow-md">
            <div class="p-5 sm:p-5">
                <div class="grid gap-4 sm:grid-cols-3 sm:gap-5">
                    <x-shopfolio::forms.group class="sm:col-span-1" label="shopfolio::layout.forms.label.sku" for="sku" :error="$errors->first('sku')">
                    ...
                    <x-shopfolio::forms.group class="sm:col-span-1" label="shopfolio::layout.forms.label.safety_stock" for="security_stock">
                        <x-shopfolio::forms.input wire:model.defer="securityStock" id="security_stock" type="number" min="0" autocomplete="off" />
                    </x-shopfolio::forms.group>
                </div>
            </div>
    ...
    <div class="mt-10 sm:mt-0">
        <div>
            <h3 class="text-lg leading-6 font-medium text-secondary-900 dark:text-white">
                {{ __('shopfolio::pages/products.inventory.stock_title') }}
            </h3>
            <p class="mt-1 max-w-2xl text-sm text-secondary-500 dark:text-secondary-400">
                {{ __('shopfolio::pages/products.inventory.stock_description') }}
            </p>
        </div>
    </div>
    ...
    <div class="mt-5 bg-white dark:bg-secondary-800 shadow-md rounded-md overflow-hidden">
        @if($inventories->count() > 1)
            <div class="p-4 sm:p-5 relative flex items-center justify-between border-b border-secondary-200 dark:border-secondary-700">
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    <x-shopfolio::forms.select wire:model.lazy="inventory" id="inventory" class="-ml-px block w-full pl-3 pr-9 py-2 rounded-l-none rounded-r-md" aria-label="{{ __('shopfolio::layout.forms.placeholder.select_inventory') }}">
                ...
                <div class="relative z-0 inline-flex items-center leading-5 text-secondary-700 dark:text-secondary-400">
                    <span class="block text-sm font-medium mr-4">{{ __('shopfolio::pages/products.quantity_available') }}</span>
        ...
        <div class="sm:flex sm:items-start sm:justify-between px-4 sm:px-6 mb-2 py-6">
            <div class="relative z-0 inline-flex items-center leading-5 text-secondary-700 dark:text-secondary-400">
                <span class="block text-sm font-medium mr-4">{{ __('shopfolio::pages/products.current_qty_inventory') }}</span>
            ...
            <div class="mt-5 sm:mt-0 sm:ml-4">
                <div class="lg:flex lg:items-center">
                            ...
                            <div class="flex rounded-md shadow-sm">
                                <div class="relative flex items-stretch grow focus-within:z-10">
                                    <x-shopfolio::forms.input
                                        ...
                                        type="number"
                                        aria-label="{{ __('shopfolio::layout.forms.label.stock_number_value') }}"
                                        id="stockValue"
                                        step="1"
                                        min="0"
                    ...
                    <div class="mt-5 lg:mt-0 flex items-center">
                        <x-shopfolio::buttons.primary wire:click="updateCurrentStock" wire.loading.attr="disabled" type="button">
                            <x-shopfolio::loader wire:loading wire:target="updateCurrentStock" class="text-white" />
                            {{ __('shopfolio::layout.forms.actions.update') }}
                        </x-shopfolio::buttons.primary>
                        @if($histories->isNotEmpty())
                            <div class="flex items-center pl-4 space-x-4">
                                <x-shopfolio::buttons.default wire:click="export" type="button">
                                    <x-heroicon-o-cloud-download class="w-5 h-5 -ml-1 pr-2" />
                                    {{ __('shopfolio::layout.forms.actions.export') }}
                ...
                @error('value')
                    <p class="mt-2 text-sm text-red-600">{{ __('shopfolio::messages.validation.integer') }}</p>
                @enderror
            </div>
        </div>
        @if($histories->isEmpty())
            <div class="flex flex-col items-center justify-center p-4 sm:p-6">
                ...
                <h3 class="font-medium py-5 text-secondary-400 text-xl">{{ __('shopfolio::pages/products.inventory.empty') }}</h3>
            </div>
        @else
                            ...
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-sm leading-5 font-medium text-secondary-700 dark:text-secondary-400 tracking-wider">
                                        {{ __('shopfolio::messages.date') }}
                                    </th>
                                    <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-sm leading-5 font-medium text-secondary-700 dark:text-secondary-400 tracking-wider">
                                        {{ __('shopfolio::messages.event') }}
                                    </th>
                                    <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-sm leading-5 font-medium text-secondary-700 dark:text-secondary-400 tracking-wider">
                                        {{ __('shopfolio::messages.location') }}
                                    </th>
                                    <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-right text-sm leading-5 font-medium text-secondary-700 dark:text-secondary-400 tracking-wider">
                                        {{ __('shopfolio::messages.adjustment') }}
                                    </th>
                                    <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-right text-sm leading-5 font-medium text-secondary-700 dark:text-secondary-400 tracking-wider">
                                        {{ __('shopfolio::pages/products.inventory.movement') }}
                                    </th>
                                </tr>
                            </thead>
