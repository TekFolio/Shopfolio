<div>
    <div>
        <h3 class="text-lg leading-6 font-medium text-secondary-900 dark:text-white">
            {{ __('shopfolio::pages/products.variants.title') }}
        </h3>
        <p class="mt-1 max-w-2xl text-sm text-secondary-500">
            {{ __('shopfolio::pages/products.variants.description') }}
        </p>
    </div>
    ...
    <section aria-labelledby="products_variations_heading">
        <div class="mt-5 bg-white dark:bg-secondary-800 pt-5 shadow rounded-md">
            <div class="px-4 sm:px-5 flex items-center justify-between space-x-4">
                <x-shopfolio::forms.search label="shopfolio::pages/products.variants.search_label" placeholder="shopfolio::pages/products.variants.search_placeholder" />
                <div>
                    <span class="shadow-sm rounded-md">
                        <x-shopfolio::buttons.primary wire:click="$emit('openModal', 'shopfolio-modals.add-variant', {{ json_encode([$product->id, $currency]) }})" type="button">
                            {{ __('shopfolio::pages/products.variants.add') }}
                        ...
                        <thead>
                            <tr class="border-t border-secondary-200 dark:border-secondary-700">
                                <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-xs leading-4 font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">
                                    <span class="lg:pl-2">{{ __('shopfolio::messages.variant') }}</span>
                                </th>
                                <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-xs leading-4 font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">
                                    {{ __('shopfolio::layout.tables.sku') }}
                                </th>
                                <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-xs leading-4 font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">
                                    {{ __('shopfolio::layout.tables.price') }}
                                </th>
                                <th class="px-6 py-3 border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-700 text-left text-xs leading-4 font-medium text-secondary-500 dark:text-secondary-400 uppercase tracking-wider">
                                    {{ __('shopfolio::layout.tables.current_stock') }}
                                </th>
                                ...
                                <tr>
                                    <td class="px-6 py-3 max-w-xl text-sm leading-5 font-medium text-secondary-900 dark:text-white">
                                        <div class="flex items-center space-x-3 lg:pl-2">
                                            ...
                                            <div class="flex items-center">
                                                @if($variant->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')))
                                                    <img class="h-8 w-8 rounded object-cover object-center" src="{{ $variant->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')) }}" alt="" />
                                                @else
                                    ...
                                    <td class="px-6 py-3 whitespace-no-wrap text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                                        <div class="flex items-center">
                                            ...
                                            {{ __('shopfolio::messages.in_stock') }}
                                        </div>
                                    </td>
                                    <td class="pr-6">
                                        <x-shopfolio::dropdown customAlignmentClasses="right-12 -bottom-1">
                                            <x-slot name="trigger">
                                            ...
                                            <x-slot name="content">
                                                <div class="py-1">
                                                    <a href="{{ route('shopfolio.products.variant', ['product' => $product->id, 'id' => $variant->id]) }}" class="group flex items-center px-4 py-2 text-sm leading-5 text-secondary-700 dark:text-secondary-400 hover:bg-secondary-100 dark:hover:bg-secondary-600 hover:text-secondary-900 dark:hover:text-white" role="menuitem">
                                                        <x-heroicon-s-pencil-alt class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500" />
                                                        {{ __('shopfolio::layout.forms.actions.edit') }}
                                                    </a>
                                                </div>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-3 whitespace-no-wrap text-sm leading-5 font-medium text-secondary-900">
                                        <div class="flex justify-center items-center space-x-2">
                                            ...
                                            <span class="font-medium py-8 text-secondary-400 text-xl">{{ __('shopfolio::pages/products.variants.empty') }}...</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white dark:bg-secondary-800 px-4 py-3 flex items-center rounded-b-md justify-between border-t border-secondary-200 dark:border-secondary-700 sm:px-6">
                <div class="flex-1 flex justify-between sm:hidden">
                    {{ $variants->links('shopfolio::livewire.wire-mobile-pagination-links') }}
                </div>
