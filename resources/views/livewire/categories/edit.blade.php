                ...
                <div class="mt-4">
                    <x-shopfolio::forms.group label="Parent" for="category" wire:ignore>
                        <x-shopfolio::forms.select wire:model.defer="selectedCategory" id="category" x-data="{}" x-init="function () { choices($el) }">
                            <option value="0">{{ __('shopfolio::pages/categories.empty_parent') }}</option>
                            @foreach($categories as $cat)
                                @if($cat->id !== $categoryId)
                                    ...
                                    @foreach($cat->children as $child)
                                        @includeWhen($child->id !== $categoryId, 'shopfolio::components.forms.option-category', ['name' => $cat->name, 'category' => $child])
                                    @endforeach
                                @endif
                            @endforeach
            ...
            <aside class="sticky top-6 space-y-5">
                <div class="bg-white rounded-md shadow overflow-hidden dark:bg-secondary-800">
                    <div class="p-4 sm:p-5">
                        ...
                        <div class="mt-1">
                            <livewire:shopfolio-forms.uploads.single :media="$category->getFirstMedia(config('shopfolio.system.storage.disks.uploads'))" />
                        </div>
                    </div>
                </div>
            </aside>
