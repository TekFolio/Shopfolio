...
>
    ...
    <x-slot name="title">
        {{ __('shopfolio::pages/products.modals.variants.title') }}
    </x-slot>
        ...
        <div class="overflow-y-scroll h-96">
            <div class="overflow-hidden">
                @if($inventories->isNotEmpty())
                    <div class="relative flex items-center justify-between p-4 border-b sm:px-5 border-secondary-200 dark:border-secondary-700">
                        <span class="relative z-0 inline-flex rounded-md shadow-sm">
                            ...
                            <x-shopfolio::forms.select wire:model.defer="inventory" id="inventory" class="block py-2 pl-3 -ml-px rounded-l-none pr-9 rounded-r-md" aria-label="{{ __('shopfolio::pages/products.modals.variants.select') }}">
                                <option value="0">{{ __('shopfolio::pages/products.modals.variants.select') }}</option>
                        ...
                        <div class="relative z-0 inline-flex items-center leading-5 text-secondary-700 dark:text-secondary-300">
                            <span class="block mr-4 text-sm font-medium">{{ __('shopfolio::pages/products.quantity_available') }}</span>
                ...
                <div class="flex items-center justify-between px-4 py-6 mb-2 sm:px-6">
                    <div class="relative z-0 inline-flex items-center leading-5 text-secondary-700 dark:text-secondary-300">
                        <span class="block mr-4 text-sm font-medium">{{ __('shopfolio::pages/products.current_qty_inventory') }}</span>
                    ...
                    <div class="ml-4">
                                ...
                                <div>
                                    <div class="flex rounded-md shadow-sm">
                                        <div class="relative flex items-stretch grow focus-within:z-10">
                                            <x-shopfolio::forms.input wire:model="value" type="number" aria-label="{{ __('shopfolio::layout.forms.label.stock_number_value') }}" id="stockValue" step="1" min="0" class="block w-32 rounded-r-none rounded-l-md" placeholder="12" />
                                        </div>
                        ...
                        @error('value')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ __('shopfolio::messages.validation.integer') }}</p>
                        @enderror
                    </div>
                </div>
                ...
                @if($histories->total() === 0)
                    <div class="flex flex-col items-center justify-center p-4 sm:p-6">
                        ...
                        <h3 class="py-5 text-xl font-medium text-secondary-400">{{ __('shopfolio::pages/products.inventory.empty') }}</h3>
                    </div>
                @else
                                ...
                                <table class="min-w-full">
                                    <thead>
                                        <tr class="border-b bg-secondary-50 border-secondary-200 dark:bg-secondary-700 dark:border-secondary-700">
                                            <x-shopfolio::tables.table-head>{{ __('shopfolio::messages.date') }}</x-shopfolio::tables.table-head>
                                            <x-shopfolio::tables.table-head>{{ __('shopfolio::messages.event') }}</x-shopfolio::tables.table-head>
                                            <x-shopfolio::tables.table-head>{{ __('shopfolio::messages.location') }}</x-shopfolio::tables.table-head>
                                            <x-shopfolio::tables.table-head class="text-right">{{ __('shopfolio::messages.adjustment') }}</x-shopfolio::tables.table-head>
                                            <x-shopfolio::tables.table-head class="text-right">{{ __('shopfolio::pages/products.inventory.movement') }}</x-shopfolio::tables.table-head>
                                        </tr>
                                    </thead>
    ...
    <x-slot name="buttons">
        <span class="flex w-full rounded-md shadow-sm sm:w-auto">
            <x-shopfolio::buttons.default wire:click="$emit('closeModal')" type="button">
                {{ __('shopfolio::layout.forms.actions.close') }}
            </x-shopfolio::buttons.default>
        </span>
    </x-slot>
