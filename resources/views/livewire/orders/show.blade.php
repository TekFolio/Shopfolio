<div>
    <x-shopfolio::breadcrumb back="shopfolio.orders.index">
        <svg class="shrink-0 h-5 w-5 text-secondary-400" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/>
        </svg>
        <a href="{{ route('shopfolio.orders.index') }}" class="text-secondary-500 dark:text-secondary-400 hover:text-secondary-700 focus:outline-none focus:underline transition duration-150 ease-in-out">{{ __('Orders') }}</a>
                    ...
                    @if(! $order->isCompleted())
                        ...
                        @if(! $order->isPaid())
                            <span class="hidden sm:block">
                                <x-shopfolio::buttons.danger wire:click="$emit('openModal', 'shopfolio-modals.archived-order', {{ json_encode([$order->id]) }})" type="button">
                        ...
                        <x-shopfolio::dropdown>
                            ...
                            <x-slot name="content">
                                <div class="py-1">
                                    @if($order->isPending())
                                        <x-shopfolio::dropdown-button wire:click="register" role="menuitem">
                                            {{ __('Register') }}
                                        </x-shopfolio::dropdown-button>
                                    @endif

                                    @if($order->isPending() || $order->isRegister())
                                        <x-shopfolio::dropdown-button wire:click="markPaid" role="menuitem">
                                            {{ __('Mark as paid') }}
                                        </x-shopfolio::dropdown-button>
                                    @endif

                                    @if($order->isPaid())
                                        <x-shopfolio::dropdown-button wire:click="markComplete" role="menuitem">
                                    ...
                                    @if($order->canBeCancelled())
                                        <x-shopfolio::dropdown-button wire:click="cancelOrder" role="menuitem">
            ...
            <div class="py-4 sm:pr-8">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <x-shopfolio::forms.select wire:model="perPage" class="w-20" aria-label="{{ __('Per Page items') }}">
                        ...
                        @foreach($items as $item)
                            <li class="flex-1 py-2.5">
                                <div class="flex items-center">
                                    <div class="min-w-0 flex-1 flex items-center">
                                        <div class="shrink-0">
                                            <div class="shrink-0 h-10 w-10">
                                                @if($item->product->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')))
                                                    <img class="h-10 w-10 rounded-md object-cover" src="{{ $item->product->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')) }}" alt="{{ $item->id }}" />
                                                @else
                                        ...
                                        <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-4 md:gap-4">
                                            ...
                                            <div class="hidden md:block">
                                                <span class="text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                                                    {{ shopfolio_money_format($item->unit_price_amount) }} x {{ $item->quantity }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <span class="text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                                            {{ shopfolio_money_format($item->total) }}
                                        </span>
                                    </div>
                                </div>
                            </li>
                ...
                <div class="mt-4">
                    <div class="py-4 flex">
                        ...
                        <div class="ml-4">
                            ...
                            <a href="{{ route('shopfolio.settings.payments') }}" class="text-sm text-secondary-500 dark:text-secondary-400 hover:text-secondary-400 underline">{{ __('View available methods') }}</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @if($order->shipping_method)
                        <dl>
                            ...
                            <div class="bg-secondary-100 py-4 sm:grid sm:grid-cols-3 sm:gap-4 px-4 dark:bg-secondary-700">
                                ...
                                <dd class="mt-1 text-sm font-medium text-secondary-900 dark:text-white sm:mt-0 sm:col-span-2">
                                    {{ shopfolio_money_format($order->shipping_total) }}
                                </dd>
                            </div>
                            <div class="bg-secondary-50 py-4 sm:grid sm:grid-cols-3 sm:gap-4 px-4 dark:bg-secondary-800">
                                <dt class="text-sm font-medium text-secondary-500 dark:text-secondary-400">
            ...
            <div class="py-4 sm:pr-8">
                <div class="flex justify-end">
                    <div class="w-full sm:max-w-sm space-y-1 text-right text-secondary-700 dark:text-secondary-300">
                        <div class="bg-secondary-200 rounded-md p-4 dark:bg-secondary-800">
                            ...
                            {{ shopfolio_money_format($order->fullPriceWithShipping()) }}
                        </div>
                    </div>
                </div>
                <div class="flex justify-end mt-2 text-right">
                    <x-shopfolio::dropdown>
                        ...
                        <x-slot name="content">
                            <div class="py-1">
                                <x-shopfolio::dropdown-button role="menuitem">
                                    {{ __('Send invoice') }}
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        @else
                            <div>
                                ...
                                <x-shopfolio::forms.textarea wire:model.lazy="notes" id="comment" placeholder="{{ __('Leave notes for this customer') }}" :value="$order->notes" />
                                @error('notes')
                                    <p class="mt-1 text-red-500 text-sm">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="mt-6 flex items-center justify-end space-x-4">
                                <x-shopfolio::buttons.primary wire:click="leaveNotes" wire:loading.attr="disabled" type="button">
                                    <x-shopfolio::loader wire:loading wire:target="leaveNotes" />
                                    {{ __('Send notes') }}
