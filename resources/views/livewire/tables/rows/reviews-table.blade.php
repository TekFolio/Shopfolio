<x-livewire-tables::table.cell>
    <div class="flex items-center space-x-3 lg:pl-2">
        ...
        <a href="{{ route('shopfolio.reviews.show', $row) }}" class="flex items-center">
            @if($row->reviewrateable->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')))
                <img class="h-8 w-8 rounded object-cover object-center" src="{{ $row->reviewrateable->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')) }}" alt="">
            @else
...
<x-livewire-tables::table.cell class="hidden md:table-cell whitespace-no-wrap text-sm leading-5 text-secondary-500 dark:text-secondary-400">
    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $row->approved ? 'bg-green-100 text-green-800': 'bg-orange-100 text-orange-800' }}">
        {{ $row->approved ? __('shopfolio::pages/products.reviews.published') : __('shopfolio::pages/products.reviews.pending') }}
    </span>
</x-livewire-tables::table.cell>
