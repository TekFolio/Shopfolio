<x-livewire-tables::table.cell>
    <div class="flex items-center">
        ...
        @endif
        <a href="{{ route('shopfolio.collections.edit', $row) }}" class="ml-2 truncate hover:text-secondary-700 dark:hover:text-secondary-300 font-medium">
            <span>{{ $row->name }}</span>
        </a>
    </div>
</x-livewire-tables::table.cell>
...
<x-livewire-tables::table.cell>
    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 {{ $row->isAutomatic() ? 'bg-green-100 text-green-800' :  'bg-primary-100 text-primary-800' }}">
        ...
        {{ $row->isAutomatic() ? __('shopfolio::pages/collections.automatic') : __('shopfolio::pages/collections.manual') }}
    </span>
</x-livewire-tables::table.cell>
