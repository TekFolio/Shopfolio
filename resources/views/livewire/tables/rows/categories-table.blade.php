<x-livewire-tables::table.cell>
    <div class="flex items-center space-x-3 lg:pl-2">
        ...
        <div class="flex items-center">
            @if($row->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads')))
                <img class="h-8 w-8 rounded object-cover object-center" src="{{ $row->getFirstMediaUrl(config('shopfolio.system.storage.disks.uploads'))}}" alt="" />
            @else
            ...
            @endif
            <a href="{{ route('shopfolio.categories.edit', $row) }}" class="ml-2 truncate hover:text-secondary-600 font-medium">
                <span>
                    {{ $row->name }} @if($row->parent_id)<span class="text-secondary-500 font-normal dark:text-secondary-400">{{ __('shopfolio::pages/categories.parent', ['parent' => $row->parent_name]) }}</span>@endif
                </span>
            </a>
        </div>
    </div>
