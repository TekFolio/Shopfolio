@if ($paginator->hasPages())
    ...
    <div role="navigation">
        <span class="relative z-0 inline-flex shadow-sm">
            ...
            @else
                <x-shopfolio::buttons.default type="button" class="relative inline-flex px-2 rounded-r-none rounded-l-md" wire:click="previousPage" rel="prev" aria-label="@lang('pagination.previous')">
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <x-shopfolio::buttons.default type="button" class="-ml-px relative rounded-l-none rounded-r-none focus:z-10" wire:click="gotoPage({{ $page }})">{{ $page }}</x-shopfolio::buttons.default>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <x-shopfolio::buttons.default type="button" class="-ml-px relative px-2 rounded-l-none rounded-r-md" wire:click="nextPage" rel="next" aria-label="{{ __('Next') }}">
                    <x-heroicon-s-chevron-right class="h-5 w-5" />
                </x-shopfolio::buttons.default>
            @else
                <x-shopfolio::buttons.default type="button" class="-ml-px relative px-2 rounded-l-none rounded-r-md focus:z-10 opacity-50 cursor-not-allowed" aria-disabled="true" aria-label="{{ __('Next') }}">
                    <x-heroicon-s-chevron-right class="h-5 w-5" />
                </x-shopfolio::buttons.default>
            @endif
        </span>
