                    ...
                    <div class="focus:outline-none">
                        ...
                        <div class="flex items-center justify-between space-x-2">
                            <span class="inline-flex text-xs leading-4 text-secondary-500 dark:text-secondary-400">
                                {{ $address->type === 'shipping' ? __('shopfolio::pages/customers.addresses.shipping') : __('shopfolio::pages/customers.addresses.billing') }}
                            </span>
                            @if($address->is_default)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    {{ __('shopfolio::pages/customers.addresses.default') }}
                                </span>
                            @endif
                        </div>
        ...
        @empty
            <div class="sm:col-span-3 relative rounded-lg bg-white px-6 py-10 sm:py-12 shadow flex flex-col items-center dark:bg-secondary-800">
                ...
                <div class="mt-5 w-full sm:max-w-md space-y-2 text-center">
                    <p class="text-base leading-6 text-secondary-900 font-medium dark:text-white">{{ __('shopfolio::pages/customers.addresses.customer') }}</p>
                    <p class="text-sm text-secondary-500 dark:text-secondary-400">{{ __('shopfolio::pages/customers.addresses.empty_text') }}</p>
                </div>
            </div>
        @endforelse
    </div>
