<div x-data="{ currentTab: 'html'}">
    <x-shopfolio::breadcrumb back="shopfolio.settings.users">
        <x-heroicon-s-chevron-left class="shrink-0 h-5 w-5 text-secondary-400" />
        <x-shopfolio::breadcrumb.link :link="route('shopfolio.settings.mails')" title="Email configuration" />
    ...
    <div class="mt-6 pb-10">
        <div x-show="currentTab === 'html'" class="grid grid-cols-1 gap-y-8 sm:grid-cols-2 sm:gap-x-5 sm:gap-y-6 md:grid-cols-3 lg:col-span-3">
            @foreach($skeletons->get('html') as $name => $subSkeletons)
                <button wire:click='$emit("openModal", "shopfolio-modals.choose-skeleton", {{ json_encode([$name, 'html', $subSkeletons]) }})' type="button" class="block overflow-hidden group text-left">
                    <figure>
                        <div class="relative rounded overflow-hidden transition transform duration-150 ease-in-out">
                            @if (file_exists(public_path("shopfolio/images/skeletons/html/{$name}.png")))
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/html/'.$name.'.png' ) }}" alt="{{ $name }}">
                            @elseif(file_exists(public_path( "shopfolio/images/skeletons/html/{$name}.jpg")))
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/html/'.$name.'.jpg' ) }}" alt="{{ $name }}">
                            @else
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/no-image.png' ) }}" alt="{{ $name }}">
                            @endif
        ...
        <div x-cloak x-show="currentTab === 'markdown'" class="grid grid-cols-1 gap-y-8 sm:grid-cols-2 sm:gap-x-5 sm:gap-y-6 md:grid-cols-3 lg:col-span-3">
            @foreach($skeletons->get('markdown') as $name => $subSkeletons)
                <button wire:click='$emit("openModal", "shopfolio-modals.choose-skeleton", {{ json_encode([$name, 'markdown', $subSkeletons]) }})' type="button" class="block group text-left">
                    <figure>
                        <div class="relative rounded overflow-hidden transition transform duration-150 ease-in-out">
                            @if (file_exists(public_path("shopfolio/images/skeletons/markdown/{$name}.png")))
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/markdown/'.$name.'.png' ) }}" alt="{{ $name }}">
                            @elseif(file_exists(public_path( "shopfolio/images/skeletons/markdown/{$name}.jpg")))
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/markdown/'.$name.'.jpg' ) }}" alt="{{ $name }}">
                            @else
                                <img class="w-full h-44 object-top object-cover" src="{{ asset('shopfolio/images/skeletons/no-image.png' ) }}" alt="{{ $name }}">
                            @endif
                            <div class="absolute inset-0 flex items-center justify-center text-center rounded-md border-secondary-900 opacity-15 border"></div>
