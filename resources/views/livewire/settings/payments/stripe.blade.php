        ...
        <div class="md:col-span-1">
            <div class="px-4 sm:px-0">
                ...
                <p class="mt-4 text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                    {{ __('shopfolio::pages/settings.payment.stripe_description') }}
                </p>
            </div>
        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <h3 class="text-base leading-6 font-medium text-secondary-900 dark:text-white">
                                @if ($this->enabled)
                                    {{ __('shopfolio::pages/settings.payment.stripe_enabled') }}
                                @else
                                    {{ __('shopfolio::pages/settings.payment.stripe_disabled') }}
                                @endif
                            </h3>
                        </div>
                    </div>
                ...
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="mt-4">
                        <p class="text-secondary-500 text-sm leading-5 dark:text-secondary-400">
                            {{ __('shopfolio::pages/settings.payment.stripe_provider') }}
                            <a href="https://github.com/stripe/stripe-php" target="_blank" class="text-primary-600 hover:text-primary-500">{{ __('shopfolio::pages/settings.payment.stripe_about') }}</a>
                        </p>
                        @if(! $this->enabled)
                            <span class="mt-4 inline-flex rounded-md shadow-sm">
                                <x-shopfolio::buttons.default wire:click="enabledStripe" wire.loading.attr="disabled" type="button">
                                    <x-shopfolio::loader wire:loading wire:target="enabledStripe" class="text-secondary-600 dark:text-secondary-300" />
                                    {{ __('shopfolio::pages/settings.payment.stripe_actions') }}
    ...
    @if($this->enabled)
        ...
        <div class="mt-10 sm:mt-0">
            <div class="md:grid md:grid-cols-3 md:gap-6">
                <div class="md:col-span-1">
                    <div class="px-4 sm:px-0">
                        <h3 class="text-lg font-bold leading-6 text-secondary-900 dark:text-white">{{ __('shopfolio::messages.environment') }}</h3>
                        <p class="mt-4 text-sm leading-5 text-secondary-500 dark:text-secondary-400">
                            {{ __('shopfolio::pages/settings.payment.stripe_environment') }}
                            {{ __('shopfolio::pages/settings.payment.stripe_dashboard') }} <a href="https://dashboard.stripe.com/account/apikeys" target="_blank" class="text-primary-600 dark:text-primary-500/50">https://dashboard.stripe.com/account/apikeys</a>
                        </p>
                    </div>
                </div>
                <div class="mt-5 md:mt-0 md:col-span-2">
                    <div class="shadow rounded-md overflow-hidden">
                        <div class="px-4 py-5 sm:p-6 space-y-4 bg-white dark:bg-secondary-800">
                            <div class="grid gap-4 sm:grid-cols-6 sm:gap-6">
                                <x-shopfolio::forms.group label="shopfolio::layout.forms.label.public_key" for="public_key" class="sm:col-span-3">
                                    <x-shopfolio::forms.input wire:model.lazy="stripe_key" id="public_key" type="text" autocomplete="off" />
                                </x-shopfolio::forms.group>

                                <x-shopfolio::forms.group label="shopfolio::layout.forms.label.secret_key" for="secret_key" class="sm:col-span-3">
                                    <x-shopfolio::forms.input wire:model.lazy="stripe_secret" id="secret_key" type="text" />
                                </x-shopfolio::forms.group>
                            </div>
                        </div>
                    </div>
