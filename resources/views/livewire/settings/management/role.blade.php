...
>
    ...
    <div class="mt-3 pb-5 relative border-b border-secondary-200 space-y-4 sm:pb-0 dark:border-secondary-700">
            ...
            <div class="flex space-x-3 md:absolute md:top-3 md:right-0">
                <span class="shadow-sm rounded-md">
                    @if($role->can_be_removed)
                        <x-shopfolio::buttons.danger wire:click="$emit('openModal', 'shopfolio-modals.delete-role', {{ json_encode(['id' => $role->id]) }})" type="button">
                ...
                <span class="shadow-sm rounded-md">
                    <x-shopfolio::buttons.primary wire:click="$emit('openModal', 'shopfolio-modals.create-permission', {{ json_encode(['id' => $role->id]) }})" type="button">
        ...
        <div>
            ...
            <div class="sm:hidden">
                <x-shopfolio::forms.select x-model="currentTab" aria-label="{{ __('Selected tab') }}">
    ...
    <div class="mt-6 pb-10">
        <div x-show="currentTab === 'role'" class="bg-white shadow overflow-hidden sm:rounded-md dark:bg-secondary-800">
            @if(config('shopfolio.system.users.admin_role') === $role->name)
            ...
            <div class="px-4 py-5 sm:p-6">
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <x-shopfolio::forms.group label="Name (in lowercase)" for="name" class="sm:col-span-1" :error="$errors->first('name')" isRequired>
                        <x-shopfolio::forms.input wire:model.lazy="name" type="text" id="name" placeholder="manager" />
                    </x-shopfolio::forms.group>
                    <x-shopfolio::forms.group label="Display name" for="display_name" class="sm:col-span-1">
                        <x-shopfolio::forms.input wire:model.lazy="display_name" type="text" id="display_name" placeholder="Manager" />
                    </x-shopfolio::forms.group>
        ...
        <div x-cloak x-show="currentTab === 'users'">
            <livewire:shopfolio-settings.management.users-role :role="$role" />
        </div>
        <div x-cloak x-show="currentTab === 'permissions'">
            <livewire:shopfolio-settings.management.permissions :role="$role" />
        </div>
    </div>
