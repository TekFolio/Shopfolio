            ...
            <div class="px-4 sm:px-5 flex items-center justify-between space-x-4">
                <div>
                    <span class="shadow-sm rounded-md">
                        <x-shopfolio::buttons.primary wire:click="$emit('openModal', 'shopfolio-modals.create-value', {{ json_encode([$attribute->id]) }})" type="button">
                            {{ __('Add new value') }}
                                    ...
                                    @forelse($values as $v)
                                        <tr>
                                            <td class="pl-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-secondary-900">
                                                <x-shopfolio::forms.checkbox aria-label="{{ __('Attribute value id') }}" id="is_filterable_{{ $v->id }}" :value="$v->id" />
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                                <div class="flex-1 flex justify-end items-center space-x-4">
                                                    <button wire:click="$emit('openModal', 'shopfolio-modals.update-value', {{ json_encode([$attribute->name, $type, $v->id]) }})" wire:key="{{ $v->id }}" type="button" class="text-secondary-500 hover:text-secondary-600 dark:text-secondary-400 dark:hover:text-secondary-500">
