...
<head>
    ...
    <title>{{ __('Shop Initialization') }} | Shopfolio Admin E-commerce</title>
    <meta name="description" content="@yield('meta_description', 'Laravel Shopfolio Admin Panel')">
    <meta name="author" content="@yield('meta_author', 'Godwin Stephen')">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta name="base-url" content="{{ config('app.url') }}">
    <meta name="dashboard-url" content="{{ config('app.url').'/'.shopfolio_prefix() }}">

    @include('shopfolio::includes._favicons')
    ...
    @stack('styles')
    <link rel="stylesheet" type="text/css" href="{{ mix('/css/shopfolio.css', 'shopfolio') }}">
    <livewire:styles />

    <!-- Scripts -->
    <wireui:scripts />
    <livewire:scripts />
    <script src="{{ mix('/js/shopfolio.js','shopfolio') }}" defer></script>

    @include('shopfolio::includes._additional-styles')
</head>
<body class="text-secondary-500 font-sans transition ease-in-out duration-700 dark:text-secondary-400">
    ...
    <div class="min-h-screen bg-secondary-100 dark:bg-secondary-900">
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        <div class="shrink-0 flex items-center">
                            <x-shopfolio::application-logo class="w-auto h-9" />
                        </div>
                    </div>
            ...
            <div x-description="Mobile menu, toggle classes based on menu state." x-state:on="Open" x-state:off="closed" :class="{ 'block': open, 'hidden': !open }" class="sm:hidden hidden">
                <div class="pt-4 pb-3 border-t border-secondary-200">
                    ...
                    <div class="mt-3" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                        ...
                        <form id="sm-logout-form" action="{{ route('shopfolio.logout') }}" method="POST" style="display: none;">
                            @csrf
                        </form>
                    </div>
                </div>
        ...
        <livewire:shopfolio-initialization />
    </div>
