<div class="flex flex-col justify-between bg-primary-800 text-center w-16">
    <div>
        <div class="px-2 bg-white">
            <a class="shrink-0 flex items-center justify-center p-2" href="{{ route('shopfolio.dashboard') }}">
                <x-shopfolio::brand class="h-8 w-auto" />
            </a>
        </div>
        <div class="h-full pt-5 pb-4 overflow-y-auto">
            <ul class="mt-5 px-2 space-y-2 flex flex-col items-center">
                <x-shopfolio::menu.item
                    :href="route('shopfolio.dashboard')"
                    :active="request()->routeIs('shopfolio.dashboard')"
                    x-tooltip.raw="{{ __('shopfolio::layout.sidebar.dashboard') }}"
                >
    ...
    <div class="shrink-0 flex flex-col items-center space-y-3 pb-4">
        ...
        @can('access_setting')
            <a
                href="{{ route('shopfolio.settings.index') }}"
                class="block p-2 text-base leading-6 font-medium rounded-md text-white hover:bg-primary-700 @if(request()->routeIs('shopfolio.settings*')) bg-primary-700 @endif focus:outline-none focus:bg-primary-900 transition ease-in-out duration-150"
                x-tooltip.raw="{{ __('shopfolio::layout.account_dropdown.settings') }}"
            >
        ...
        <a
            ...
            x-tooltip.raw="{{ __('shopfolio::messages.dashboard.cards.doc_title') }}"
        >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
