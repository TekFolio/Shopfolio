...
namespace Shopfolio\Traits\Mails;
...
trait Templates
{
    ...
    {
        $file = config('shopfolio.mails.mailables_dir') . 'templates.json';
        if (! file_exists($file)) {
            if (! file_exists(config('shopfolio.mails.mailables_dir'))) {
                mkdir(config('shopfolio.mails.mailables_dir'));
            }
    ...
    public static function deleteTemplate($templateSlug): bool
    {
        ...
        if (null !== $template) {
            ...
            $template_view = 'shopfolio::mails.templates.' . $templateSlug;
    ...
    public static function updateTemplate(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $template_view = 'shopfolio::mails.templates.' . $request->templateslug;
        $template_plaintext_view = $template_view . '_plain_text';
    ...
    public static function createTemplate(Request $request)
    {
        ...
        $view = 'shopfolio::templates.' . $request->template_name;
        ...
        if (! view()->exists($view) && ! self::getTemplates()->contains('template_slug', '=', $templatename)) {
            ...
            $dir = resource_path('views/vendor/shopfolio/templates');
            ...
            return redirect()->route('shopfolio.settings.mails');
        }
    ...
    {
        return collect(config('shopfolio.mails.skeletons'));
    }

    public static function getTemplateSkeleton($type, $name, $skeleton)
    {
        $skeletonView = "shopfolio::mails.skeletons.{$type}.{$name}.{$skeleton}";
    ...
    {
        if ($template && View::exists('shopfolio::mails.templates.' . $viewPath)) {
            $viewPath = View('shopfolio::mails.templates.' . $viewPath)->getPath();
        }
    ...
    public static function previewMarkdownViewContent($simpleview, $content, $viewName, $template = false, $namespace = null)
    {
        ...
        if (file_exists($dir)) {
            ...
            return self::renderPreview($simpleview, 'shopfolio::mails.draft.' . $viewName, $template, $instance);
        }
