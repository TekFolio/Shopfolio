        ...
        return [
            Column::make('#', 'number')
                ...
                ->format(function ($value, $column, $row) {
                    return view('shopfolio::livewire.tables.cells.orders.number')->with('order', $row);
                }),
            ...
            Column::make('Status', 'status')
                ->sortable()
                ->format(function ($value, $column, $row) {
                    return view('shopfolio::livewire.tables.cells.orders.status')->with('order', $row);
                }),
            Column::make('Customer', 'user_id')
                ...
                ->sortable()
                ->format(function ($value, $column, $row) {
                    return view('shopfolio::livewire.tables.cells.orders.customer')->with('customer', $row->customer);
                }),
            Column::make('Purchased')
                ->format(function ($value, $column, $row) {
                    return view('shopfolio::livewire.tables.cells.orders.purchased')->with('order', $row);
                }),
            Column::make('Total')
                ->addClass('text-right')
                ->format(function ($value, $column, $row) {
