...
use Shopfolio\Notifications\AdminSendCredentials;
...
class CreateAdminUser extends AbstractBaseComponent
{
    ...
    public function updatedRoleId(int $id)
    {
        ...
        $this->is_admin = $chooseRole->name === config('shopfolio.system.users.admin_role');
    }
    ...
    public function rules(): array
    {
        return [
            'email' => [
                'required',
                'email',
                Rule::unique(shopfolio_table('users'), 'email'),
    ...
    {
        return view('shopfolio::livewire.settings.management.create', [
            'roles' => Role::query()
                ->select(['id', 'display_name', 'name'])
                ->where('name', '<>', config('shopfolio.system.users.default_role'))
                ->get(),
        ]);
    }
}
