...
namespace Shopfolio\Http\Livewire\Settings;

use Illuminate\Support\Facades\Cache;
use Livewire\Component;
use Livewire\WithFileUploads;
use Shopfolio\Models\System\Country;
...
class General extends Component
{
    ...
    public function store()
    {
        ...
        if ($this->logo) {
            Setting::query()->updateOrCreate(['key' => 'shop_logo'], [
                'value' => $this->logo->store('/', config('shopfolio.system.storage.disks.uploads')),
        ...
        if ($this->cover) {
            Setting::query()->updateOrCreate(['key' => 'shop_cover'], [
                'value' => $this->cover->store('/', config('shopfolio.system.storage.disks.uploads')),
                'display_name' => Setting::lockedAttributesDisplayName('shop_cover'),
                'locked' => true,
            ]);
        }

        $this->notification()->success(__('shopfolio::layout.status.updated'), __('Shop informations have been correctly updated!'));
    }
    ...
    public function deleteCover()
    {
        ...
        $this->notification()->success(__('shopfolio::layout.status.delete'), __('Shop cover have been correctly removed!'));
    }

    public function render()
    {
        return view('shopfolio::livewire.settings.general', [
