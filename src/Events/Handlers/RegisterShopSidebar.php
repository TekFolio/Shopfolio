...
namespace Shopfolio\Events\Handlers;

use Maatwebsite\Sidebar\Group;
use Maatwebsite\Sidebar\Item;
use Maatwebsite\Sidebar\Menu;
use Shopfolio\Sidebar\AbstractAdminSidebar;

class RegisterShopSidebar extends AbstractAdminSidebar
{
    public function extendWith(Menu $menu): Menu
    {
        $menu->group(__('shopfolio::layout.sidebar.shop'), function (Group $group) {
            $group->weight(20);
            $group->authorize();

            $group->item(__('shopfolio::layout.sidebar.brands'), function (Item $item) {
                $item->weight(3);
                $item->authorize($this->user->hasPermissionTo('browse_brands'));
                $item->route('shopfolio.brands.index');
                $item->icon('
            ...
            $group->item(__('shopfolio::layout.sidebar.categories'), function (Item $item) {
                $item->weight(4);
                $item->authorize($this->user->hasPermissionTo('browse_categories'));
                $item->route('shopfolio.categories.index');
                $item->icon('
            ...
            $group->item(__('shopfolio::layout.sidebar.collections'), function (Item $item) {
                $item->weight(5);
                $item->authorize($this->user->hasPermissionTo('browse_collections'));
                $item->route('shopfolio.collections.index');
                $item->icon('
                    <svg class="shrink-0 -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            $group->item(__('shopfolio::layout.sidebar.catalog'), function (Item $item) {
                $item->weight(2);
                $item->authorize($this->user->hasPermissionTo('browse_products'));
                $item->route('shopfolio.products.index');
                $item->icon('
                    <svg class="shrink-0 -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            $group->item(__('shopfolio::layout.sidebar.customers'), function (Item $item) {
                $item->weight(6);
                $item->authorize($this->user->hasPermissionTo('browse_customers'));
                $item->route('shopfolio.customers.index');
                $item->icon('
                    <svg class="shrink-0 -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            $group->item(__('shopfolio::layout.sidebar.reviews'), function (Item $item) {
                $item->weight(7);
                $item->authorize($this->user->hasPermissionTo('browse_products'));
                $item->route('shopfolio.reviews.index');
                $item->icon('
                    <svg class="shrink-0 -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
