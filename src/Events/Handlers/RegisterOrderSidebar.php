...
class RegisterOrderSidebar extends AbstractAdminSidebar
{
    public function extendWith(Menu $menu): Menu
    {
        ...
        $menu->group(__('shopfolio::layout.sidebar.shop'), function (Group $group) use ($count) {
            $group->weight(20);
            $group->authorize();

            $group->item(__('shopfolio::layout.sidebar.orders'), function (Item $item) use ($count) {
                $item->weight(1);
                $item->authorize($this->user->hasPermissionTo('browse_orders'));
                if ($count > 0) {
                    $item->badge($count);
                }
                $item->route('shopfolio.orders.index');
                $item->icon('
            ...
            $group->item(__('shopfolio::layout.sidebar.discounts'), function (Item $item) {
                $item->weight(9);
                $item->authorize($this->user->hasPermissionTo('browse_discounts'));
                $item->route('shopfolio.discounts.index');
                $item->icon('
